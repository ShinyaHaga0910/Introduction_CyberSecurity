...
- `ERROR`を含む行だけを抽出する。
- 各行の先頭に、元の`incident.log`での行番号を付ける。
- 形式は`行番号:元の行全体`とする。例は`12:2026-09-01T09:00:00Z ERROR example`である。
- 本文を手入力せず、検索結果を標準出力から`notes/errors.txt`へ保存する。

`recent.txt`には、`case01/logs/incident.log`の最後の5行を、順序と内容を変えずに保存する。

観察候補: `man grep`、`grep --help`。行番号を付けるoptionと、標準出力をfileへ保存する方法を確認する。
