...
Linux未経験の学生が、コンピューター、OS、Linuxの機能を理由と仕組みから学ぶA4縦の教科書である。章別Markdownを内容の正本とし、専用の説明図20点と歴史写真1点を組み込んだ。PPTXは作成していない。公開Lab v1.5.0、CloudFormation、install、採点スクリプトは変更していない。
...
PDFは、教科書53ページ、練習11ページ、課題9ページ、参照資料7ページである。合計80ページである。今回変更した第12章末と巻末まとめを画像化し、文字の重なり、見切れ、改ページを目視で確認した。その他のページは前版で確認済みである。
...
静的に確認済みなのは、12章と巻末まとめ、本文参照図20点、歴史写真1点の存在、MarkdownからHTML/PDFへの変換、A4寸法、文字抽出、日本語フォント埋め込み、P27判定・M39判定・M6のUbuntu 2件／CloudShell 4件との対応、公開版P/M本文とのSHA-256一致である。
