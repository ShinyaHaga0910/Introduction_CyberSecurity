...
更新: 2026-09-25。状態: **日本語版v1.15.0を改訂し、再生成・静的再検証済み。実機検証と学生試読は未実施。**

Linux未経験の学生が、コンピューター、OS、Linuxの機能を理由と仕組みから学ぶA4縦の教科書である。章別Markdownを内容の正本とし、専用の説明図20点と歴史写真1点を組み込んだ。PPTXは作成していない。公開Lab v1.5.1では学生向け文面と配布版指定を更新したが、AWSリソース構成、演習初期状態、採点条件は変更していない。
...
教材v1.9.0では、第8章から演習固有のPID 1の警告、バックグラウンド実行に関する警告、対象プロセスを終了する手順の節を削除した。`/proc`の例を現在のシェルのPIDで完結する形に変え、章末3問に答えを追加した。

教材v1.10.0では、第9章の`apt upgrade`禁止文と旧9.3節を削除した。`apt update`、`apt upgrade`、`apt install`の役割を区別し、学生が`apt upgrade`を実行できる説明に改めた。旧9.6節の演習固有の注意を削除し、章末3問に答えを追加した。

教材v1.11.0では、第10章のP4/M4に固有の説明3か所を削除した。`systemctl cat`の表示対象を公式マニュアルに合わせて明確にし、章末3問に答えを追加した。

教材v1.12.0では、第11章の演習固有の待受説明を削除した。11.5節は待受とHTTP応答の違いを説明し、11.8節はWebサービスの一般的な処理の流れへ改めた。ログの説明から採点スクリプト固有の記述を除き、章末3問に答えを追加した。

教材v1.13.0では、第12章をSSHの一般的な解説として再構成した。接続元・接続先、ホスト認証とユーザー認証、鍵の置き場所、接続名と通信経路、遠隔コマンド、SCP、転送後の内容と属性の確認を順に説明する。特定のLab環境と採点方法への言及を本文から除き、章末5問に答えを付けた。

教材v1.14.0では、第13章を廃止した。M7の解法順序と追加のトラブルシューティング問題は教科書から除き、課題に依存しない短い「本書のまとめ」を巻末に置いた。手順付きP7は追加せず、M7の問題文と採点条件は変更していない。

教材v1.15.0では、全12章の章末問題をまとめ、解答を次のページから掲載する。第8章へサービス・Main PID・プロセスの最小説明を追加し、第10章へ進む前にP3を解ける順にした。第4・5章のSSH先取り設問を削除し、第5章の遠隔シェルの説明を第12章へ移した。P3の二段階学習とP5の待受アドレス確認を明示した。
...
- 本文で使用する説明図20点、再利用に備えて保持する図1点、旧版PNG 21点: [`assets/figures/`](./assets/figures/)
...
PDFは、教科書61ページ、練習11ページ、課題9ページ、参照資料7ページである。合計88ページである。全12章で章末問題の次ページから解答が始まることを文字抽出で確認し、第8・12章の問題・解答ページを画像で確認した。

図の生成元21点は、内容に応じてD2、Typst、Freeze + Typstへ分類した。同じ配色、字体、枠線、余白の規則で統一している。旧第13章のfig19は生成元と図を保持するが、本文からは参照しない。旧版PNG 21点も比較と復元のために保持している。
...
2. [12章と巻末まとめの章・節別設計](./design/2026-09-18_ubuntu_os_materials_storyboard.md)
...
静的に確認済みなのは、12章と巻末まとめ、本文参照図20点、歴史写真1点の存在、MarkdownからHTML/PDFへの変換、A4寸法、文字抽出、日本語フォント埋め込み、P27判定・M39判定・M6のUbuntu 2件／CloudShell 4件との対応、公開Lab v1.5.1のP/M本文とのSHA-256一致である。
