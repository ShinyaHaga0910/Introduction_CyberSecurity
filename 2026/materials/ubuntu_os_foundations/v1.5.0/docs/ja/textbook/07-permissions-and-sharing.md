...
**図7-1　プロセスの資格情報から u/g/o のどのクラスを適用するかを選択し、その欄の rwx で操作の可否を判断する流れ。** 実際には拡張アクセス制御（ACL）等も存在し得るが、本演習では基本アクセス権を中心に扱う。
...
全権限を意味する `777` は、第三者である「その他（others）」に対しても無条件で書き込みや削除を許可してしまう。せっかくアクセス権や共有グループを設計した意味を完全に失わせるため、本演習環境では決して使用してはならない。
...
ディレクトリの所有者やアクセス権を変更しても、その配下にすでに存在する既存ファイルやサブディレクトリの属性が自動的に一括変更されるわけではない。`-R` オプションによって再帰的に配下全体を変更することも可能であるが、想定外のファイルまで意図せず書き換えてしまう危険を伴う。対象項目が限定されている本演習では、対象を1件ずつ明示して変更し、`stat` コマンドで結果を確認することを基本とする。
...
ディレクトリに **setgid ビット**（Set Group ID）を設定しておくと、そのディレクトリ配下で新規作成されるファイルやサブディレクトリが、作成者の主グループではなく、親ディレクトリの所有グループを自動的に引き継ぐようになる。本演習で共有先 `/srv/jdu-share` を `root:ops 2775` に設定する理由はここにある。
...
演習P2および課題M2では、共有ディレクトリに付与された setgid と、新規作成したサンプルファイルが期待どおりのアクセス権モードになる環境を確認する。これら2つは異なる役割を担う仕組みであり、混同してはならない。
...
アクセス拒否の動作を確認するのは、失敗させること自体が目的ではない。「どのユーザー／グループの資格情報で、所有者・グループ・その他のどのクラスが選択され、どの権限が不足していたのか」という拒否の理由を論理的に説明できるようにするために行う。エラーが出たからといって安易に広範なアクセス権（`777` など）を付与して問題を隠蔽するのではなく、セキュリティ要件に沿った最小限の権限設定が正しく機能しているかを検証することが極めて重要である。

## 章末確認

1. アクセス権モード `2775` の各桁（2、7、7、5）が表す意味をそれぞれ具体的に説明せよ。
2. setgid が設定されたディレクトリ配下で新規ファイルを作成した際、親ディレクトリから引き継がれる属性と、引き継がれない属性をそれぞれ挙げよ。
3. root 権限でファイルを正常に読めたとしても、一般のサービスユーザーに読み取り権限がある証明にならない理由を説明せよ。
