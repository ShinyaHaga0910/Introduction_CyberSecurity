...
def build_html(title: str, sources: list[Path], output: Path):
    parts = ["<!doctype html><html lang='ja'><head><meta charset='utf-8'>", f"<title>{html.escape(title)}</title><style>{CSS}</style></head><body>",
             f"<section class='cover'><h1>{html.escape(title)}</h1><p class='meta'>Ubuntu Server 24.04 LTS / JDU Cyber Security<br>2026-09-21</p></section>"]
    for src in sources:
...
def cover_story(title: str, subtitle: str):
    ...
    story.extend([Spacer(1, 60*mm), Paragraph("Ubuntu Server 24.04 LTS / 公開Lab v1.5.0<br/>JDU Cyber Security / 2026-09-21", ParagraphStyle("Meta", fontName="NotoSansJP", fontSize=9.5, leading=15, alignment=TA_CENTER, textColor=MUTED)), PageBreak()])
    return story
