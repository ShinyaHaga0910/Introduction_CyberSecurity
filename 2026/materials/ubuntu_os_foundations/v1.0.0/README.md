...
更新: 2026-09-18。状態: **日本語版をAntigravityで校閲し、再生成・静的再検証済み。実機検証と学生試読は未実施。**

Linux未経験の学生が、演習に必要なOSの機能を理由と仕組みから学ぶA4縦の教科書である。章別Markdownを内容の正本とし、専用の説明図19点を組み込んだ。PPTXは作成していない。公開Lab v1.5.0、CloudFormation、install、採点スクリプトは変更していない。
...
- 説明図19点: [`assets/figures/`](./assets/figures/)
- HTML 4冊: [`output/html/`](./output/html/)
- A4 PDF 4冊: [`output/pdf/`](./output/pdf/)
- 多言語化の管理: [`localization/`](./localization/)
- 再生成手順: [`BUILD.md`](./BUILD.md)

PDFは、教科書51ページ、練習11ページ、課題9ページ、参照資料7ページである。合計78ページである。全78ページを画像化し、文字や図の重なり、見切れ、不自然な空白、図とコードの可読性を目視で確認した。
...
静的に確認済みなのは、13章と19図の存在、MarkdownからHTML/PDFへの変換、A4寸法、文字抽出、フォント埋め込み、P27判定・M39判定・M6のUbuntu 2件／CloudShell 4件との対応、公開版P/M本文とのSHA-256一致である。
