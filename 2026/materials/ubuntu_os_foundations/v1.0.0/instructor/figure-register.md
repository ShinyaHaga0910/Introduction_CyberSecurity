...
すべて本教材用の自作図。生成元: `build/generate_figures.py`。確認日: 2026-09-18。

| 図 | 参照章 | 解消する疑問 | 正確さの注意 |
| --- | --- | --- | --- |
| fig01 | 1 | OSはhardwareとprogramの間で何をするか | CPU内部処理へ踏み込まない |
| fig02 | 1 | GUI/CLIとkernelの関係 | GUI→CLIを必須経路にしない |
...
19点をcontact sheetで確認済み。各図は異なる問いに専用化され、本文の参照と一致する。最終判定はA4 PDF内の実寸で再確認する。
