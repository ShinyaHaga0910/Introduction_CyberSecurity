...
for script in \
  ...
  "$root_dir/tests/mock-progress/curl"; do
  bash -n "$script"
done
...
assert parameters["LabVersion"]["AllowedValues"] == ["v1.4.0"]
...
assert 'VERSION="v1.4.0"' in installer
...
assert "jdu-check M1 --submit" in guide
assert '/opt/jdu-lab/bin/jdu-progress submit "$mission" "$PASS_COUNT" "$total"' in checker
