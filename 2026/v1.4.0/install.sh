...
VERSION="v1.4.0"
STACK_NAME="jdu-intro-cybersecurity-2026-v140"
...
if [[ -n "$PROGRESS_ENDPOINT" ]]; then
  ...
  if [[ -r "$progress_identity_file" ]]; then
    ...
    printf 'JDU_PROGRESS_SERVER_ID=%s\nJDU_PROGRESS_SERVER_TOKEN=%s\n' "$progress_server_id" "$progress_server_token" > "$progress_identity_file"
...
if [[ -n "$progress_server_id" ]]; then
  ...
  printf '%s\n' 'Send a result from Ubuntu with: jdu-check M1 --submit'
fi
