# Introduction CyberSecurity Lab 2026 v1.5.1
...
```bash
curl -fsSLO https://raw.githubusercontent.com/ShinyaHaga0910/Introduction_CyberSecurity/main/2026/v1.5.1/install.sh
...
完全手順付き演習P1～P6は[GUIDED_PRACTICE.md](GUIDED_PRACTICE.md)を参照します。各Pを終えた後、同じ技能を別の対象へ適用する自力課題M1～M6へ進みます。M0と統合課題M7にはPを設けません。
...
初回構築直後に学生がresetする必要はありません。P1～P6とM1～M7は、学生が何もしていない状態では0件PASSになるよう設計しています。

P6とM6はCloudShellとUbuntuの両方で`jdu-check P6`または`jdu-check M6`を実行します。Dashboardでは、それぞれUbuntu 2件、CloudShell 4件を分けて扱います。両方が全件PASSになったときだけ完了です。
...
| `GUIDED_PRACTICE.md` | P1～P6の完全なcommand手順 |
...
```bash
curl -fsSL \
  https://raw.githubusercontent.com/ShinyaHaga0910/Introduction_CyberSecurity/main/2026/v1.5.1/teacher/install-teacher.sh \
...
- P3を第8章のプロセス操作と第9章のパッケージ操作に分けて学ぶ順序を明記しました。
- P5でループバック待受と全アドレス待受の違いを、`ss`の結果から確認する指示を追加しました。
- M3の事前学習順を明記しました。課題、初期状態、採点条件は変更していません。
- 学生用CloudFormationと初期化スクリプトの版指定・チェックサムを更新しました。AWSリソース構成はv1.5.0と同じです。既存の教員用進捗サーバーを作り直す必要はありません。

## v1.5.0変更点

- M1～M6と同じ技能を別resourceで練習するP1～P6を追加しました。M7は統合課題のため、対応するPを設けません。
- P1～P6には、`cd`、`pwd`、編集対象directoryへの移動を含む完全なstep-by-step手順があります。
- PとMはdirectory、user/group、service、port、file名を分離しています。Pを完了してもMはPASSになりません。
- `jdu-check P1`～`jdu-check P6`と`jdu-reset P1`～`jdu-reset P6`を追加しました。
- P6もUbuntu側とCloudShell側を分けて判定・送信します。
- DashboardへGuided P1～P6の進捗欄を追加しました。自力課題M0～M7の集計は維持します。

## v1.4.3から継続する設計

- 初期構築時に`tree`をinstallします。
- Mission本文で、管理user、演習用user、service用userを明確に区別しました。
- `jdu-check`と`jdu-reset`を`ssm-user`以外で実行した場合、誤った採点をせず、戻り方を表示します。
- M2は、誤って`ops`に所属する`jduviewer`を削除し、`jduops`を追加する課題に変更しました。
- M3の3 processを通常のsystemd unitとして構築し、初期起動を検証してから環境構築を完了します。
- M5へlistener、PID、service userのread確認方法を追記しました。
- M6のUbuntu結果とCloudShell結果を別々に保存し、Dashboardで合計`6/6`まで表示します。
- CloudShell側の`jdu-check M6`も結果を自動送信します。
