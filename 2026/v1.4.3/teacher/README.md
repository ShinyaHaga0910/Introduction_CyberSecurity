...
The installer prints the one-line student installation command. The current progress endpoint and registration key are fixed in the v1.4.3 student installer, so students do not enter either value.
...
The table stores a random server ID, EC2 instance metadata, the latest M0-M7 scores, and timestamps. M6 stores Ubuntu (2 checks) and CloudShell (4 checks) separately. The dashboard counts M6 complete only when both parts are complete, and shows their combined score out of 6. It does not store student names or email addresses. Use `jdu-progress id` on a student server to display its server ID.
