...
def dashboard(event):
    ...
    for server in scan_servers():
        ...
        completed = sum(1 for name in [f"M{i}" for i in range(8)] if missions.get(name, {}).get("M", {}).get("complete", {}).get("BOOL") is True)
        cells = []
        for name in [f"M{i}" for i in range(8)]:
            score = missions.get(name, {}).get("M")
            if not score:
                cells.append(f'<td class="missing">{name}<br>—</td>')
            else:
                passed = score.get("passed", {}).get("N", "0")
                total = score.get("total", {}).get("N", "0")
                css = "done" if score.get("complete", {}).get("BOOL") else "partial"
                cells.append(f'<td class="{css}">{name}<br>{html.escape(passed)}/{html.escape(total)}</td>')
        rows.append((attr_s(server, "serverId"), f"<tr><td><strong>{html.escape(attr_s(server, 'serverId'))}</strong><br><span>{html.escape(attr_s(server, 'hostname'))}</span><br><small>{html.escape(attr_s(server, 'instanceId'))}</small></td><td><strong>{completed}/8</strong><div class=bar><i style=\"width:{completed * 12.5}%\"></i></div></td>{''.join(cells)}<td>{html.escape(attr_s(server, 'updatedAt'))}</td></tr>"))
    rows.sort(key=lambda pair: pair[0])
    body_rows = "".join(row for _, row in rows) or '<tr><td colspan="11">まだ進捗報告はありません。</td></tr>'
    page = f"""<!doctype html><html lang=ja><head><meta charset=utf-8><meta name=viewport content=\"width=device-width,initial-scale=1\"><meta http-equiv=refresh content=30><title>JDU Linux Lab Progress</title><style>body{{font-family:system-ui,sans-serif;margin:24px;background:#f4f6f8;color:#18212b}}h1{{font-size:1.35rem}}p{{color:#52606d}}.wrap{{overflow:auto;background:white;border:1px solid #d9e2ec;border-radius:10px}}table{{border-collapse:collapse;width:100%;min-width:1100px}}th,td{{padding:10px;border-bottom:1px solid #e6eaf0;text-align:center}}th:first-child,td:first-child{{text-align:left}}th{{background:#edf2f7;position:sticky;top:0}}td span,small{{color:#66788a}}.done{{background:#e5f7ea;color:#176b32}}.partial{{background:#fff3d6;color:#815500}}.missing{{color:#8997a5}}.bar{{height:8px;background:#e4e9ee;border-radius:8px;margin-top:6px}}.bar i{{display:block;height:100%;background:#238636;border-radius:8px}}footer{{margin-top:12px;font-size:.85rem;color:#66788a}}</style></head><body><h1>JDU Linux Lab 進捗</h1><p>サーバー単位の最新結果です。30秒ごとに更新します。</p><div class=wrap><table><thead><tr><th>Server</th><th>Progress</th>{''.join(f'<th>M{i}</th>' for i in range(8))}<th>Last report (UTC)</th></tr></thead><tbody>{body_rows}</tbody></table></div><footer>閲覧URLは一定時間で失効します。学生名・メールアドレスは保存しません。</footer></body></html>"""
