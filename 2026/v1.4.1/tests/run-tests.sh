...
assert parameters["LabVersion"]["AllowedValues"] == ["v1.4.1"]
...
assert 'VERSION="v1.4.1"' in installer
...
assert '"$progress_command" submit "$mission" "$PASS_COUNT" "$total"' in checker
...
printf '%s\n' 'PASS jdu-check reports automatically and --no-submit suppresses communication'
