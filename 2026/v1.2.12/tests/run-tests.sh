...
for script in \
  ...
  "$root_dir/tests/mock-m5/sudo" \
  "$root_dir/tests/mock-m6/ssh" \
  "$root_dir/tests/mock-m6/ssh-keygen"; do
  bash -n "$script"
done
...
[[ ! -e "$root_dir/scripts/jdu-probe" ]]
printf '%s\n' 'PASS eight-Mission interface and retired Ubuntu probe'
...
m6_key="$m6_cloud_home/id_ed25519"
m6_known_hosts="$m6_cloud_home/known_hosts"
mkdir -p "$m6_cloud_workspace" "$m6_remote_directory"
printf '%s\n' private-test-key > "$m6_key"
printf '%s\n' 'ssh-ed25519 AAAAM6TEST' > "$m6_key.pub"
chmod 0600 "$m6_key"
printf '%s\n' 'i-m6test ssh-ed25519 AAAAHOST' > "$m6_known_hosts"
...
chmod 0755 "$root_dir/tests/mock-m6/ssh" "$root_dir/tests/mock-m6/ssh-keygen"
m6_cloud_output="$(M6_REMOTE_DIR="$m6_remote_directory" JDU_SSH_KEY_PATH="$m6_key" JDU_M6_WORKSPACE="$m6_cloud_workspace" JDU_KNOWN_HOSTS_PATH="$m6_known_hosts" PATH="$root_dir/tests/mock-m6:$PATH" HOME="$m6_cloud_home" bash "$root_dir/scripts/jdu-cloudcheck" mission M6)"
grep -Fxq 'RESULT    10 / 10 items cleared' <<<"$m6_cloud_output"
grep -Fxq 'PASS      10' <<<"$m6_cloud_output"
grep -Fxq 'FAIL      0' <<<"$m6_cloud_output"
rm -rf -- "$m6_cloud_home"
printf '%s\n' 'PASS M6 CloudShell checker verifies keys, tunnel, transfer, host key, and rejection'
...
assert parameters["LabVersion"]["AllowedValues"] == ["v1.2.12"]
...
zero_action_contract = {
    ...
    "M6": "the transferred file and remote-command result are intentionally absent",
    "M7": "The teacher unit is loaded, inactive, and disabled; content, writer membership, and permissions are intentionally unfinished",
}
...
required_ids = {
    ...
    "M6": ["M6-SSH-01", "M6-SSH-04", "M6-SSH-05", "M6-XFER-01", "M6-REMOTE-01"],
    "M7": ["M7-UNIT-02", "M7-SVC-01", "M7-SOCK-01", "M7-HTTP-01", "M7-LOG-01", "M7-PERM-03", "M7-INT-02"],
}
...
assert "remote-success.env" not in checker
assert "M6-NEG-" not in checker
assert set(re.findall(r"M6-[A-Z]+-[0-9]+", checker)) == {
    "M6-SSH-01", "M6-SSH-02", "M6-SSH-03", "M6-SSH-04", "M6-SSH-05",
    "M6-XFER-01", "M6-XFER-02", "M6-REMOTE-01",
}
assert set(re.findall(r"M7-[A-Z]+-[0-9]+", checker)) == {
    "M7-UNIT-01", "M7-UNIT-02",
    "M7-SVC-01", "M7-SVC-02", "M7-SVC-03",
    "M7-SOCK-01", "M7-HTTP-01", "M7-LOG-01",
    "M7-PERM-01", "M7-PERM-02", "M7-PERM-03",
    "M7-INT-01", "M7-INT-02",
}
assert "JDU SSH transfer test" in checker
assert "remote-result.txt" in checker
assert "BASE_URL/scripts/jdu-probe" not in setup
...
assert "jdu-final.service.template" not in setup
assert "/opt/jdu-lab/fixtures/m7/jdu-final.service" in setup
assert "User=jdufinal" in setup
assert "WorkingDirectory=/srv/jdu-final" in setup
assert "--address 127.0.0.1 --port 8090 --content /srv/jdu-final/index.txt" in setup
assert "install -o root -g root -m 0644 /opt/jdu-lab/fixtures/m7/jdu-final.service /etc/systemd/system/jdu-final.service" in fixture
assert 'rm -f -- /etc/systemd/system/jdu-final.service' not in fixture
assert "M7-EVID-" not in checker
assert "jdu-lab/m7/evidence.env" not in checker
assert "REQUEST path=/m7-check" in checker
assert '"_SYSTEMD_INVOCATION_ID=$invocation_id"' in checker
assert "Mission M7 (13 questions)" in checker
