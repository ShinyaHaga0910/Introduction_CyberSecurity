...
grep -Fxq 'Mission M0: 6/6 PASS, 0 NOT_READY, 0 FAIL' <<<"$m0_complete_output"
grep -Eq '^Environment: [0-9]+ READY, [0-9]+ WARN, [0-9]+ ERROR \(not student questions\)$' <<<"$m0_complete_output"
...
set -e
[[ "$(grep -Ec '^NOT_READY[[:space:]]+M0-OBS-' <<<"$m0_initial_output")" -eq 6 ]]
grep -Fxq 'Mission M0: 0/6 PASS, 6 NOT_READY, 0 FAIL' <<<"$m0_initial_output"
rm -rf -- "$m0_test_home"
printf '%s\n' 'PASS M0 reports exactly six student questions separately from environment readiness'
...
assert parameters["LabVersion"]["AllowedValues"] == ["v1.2.2"]
...
assert "Mission M0 required questions (6)" in checker
assert "Environment readiness (not student questions)" in checker
assert "Environment: %d READY, %d WARN, %d ERROR (not student questions)" in checker
