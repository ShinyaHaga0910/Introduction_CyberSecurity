...
LabCheckは状態を修正しない。`FAIL`または`NOT_READY`では、表示された観察対象を自分で確認する。
...
LabCheckの`M0-ENV-*`は、教員が用意した環境の準備状態である。学生が回答する問題数には含めない。`M0-OBS-01`～`M0-OBS-06`が学生の6問である。
...
`app.conf`と`incident.log`の内容を変更しない。`errors.txt`には`ERROR`行だけを行番号付きで保存する。`recent.txt`には`incident.log`の最後の5行を保存する。
